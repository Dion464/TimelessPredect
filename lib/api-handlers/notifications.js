...
module.exports = async (req, res) => {
  ...
  try {
    ...
    if (req.method === 'PATCH') {
      // Mark notifications as read or claimed
      const { recipient, notificationIds, markClaimed, marketId } = req.body;

      if (!recipient) {
        return res.status(400).json({ error: 'recipient address is required' });
      }

      // If markClaimed is true and marketId is provided, mark by marketId
      if (markClaimed && marketId) {
        ...
        return res.status(200).json({
          success: true,
          message: 'Notification marked as claimed'
        });
      }

      // Otherwise, mark as read by notificationIds
